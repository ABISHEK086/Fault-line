...
export default function VulnerabilityDetailPage() {
  ...
  useEffect(() => {
    api.blastRadius(params.id).then(setData).catch((e: ApiError) => setError(e.message));
  }, [params.id]);
  ...
  if (!data) return <LoadingBanner label="Computing blast radius" />;

  const teamGroups = data.services.reduce<Record<string, typeof data.services>>((acc, svc) => {
  ...
  return (
    <div>
      <Link href="/vulnerabilities" className="font-mono text-xs text-muted hover:text-steady">
        ← all vulnerabilities
      </Link>

      <div className="mt-3 flex flex-wrap items-center gap-3">
        <h1 className="font-mono text-2xl font-semibold text-paper">{data.cve}</h1>
        <SeverityBadge severity={data.severity} />
      </div>
      <p className="mt-2 max-w-2xl text-sm text-muted">{data.summary}</p>
      <p className="mt-1 font-mono text-xs text-muted">
        Epicenter package: <span className="text-paper">{data.vulnerablePackage}</span>
      </p>
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[1.1fr_1fr]">
        <div className="rounded-lg border border-line bg-panel p-4">
          ...
          <p className="mt-2 text-center text-xs text-muted">
            Click a service to trace its dependency path back to the epicenter.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div>
            ...
            <div className="mt-3 flex flex-col gap-4">
              {Object.entries(teamGroups).map(([team, services]) => (
                <div key={team}>
                  <p className="font-mono text-sm text-paper">{team}</p>
                  <ul className="mt-1.5 flex flex-col gap-1.5">
                    {services.map((svc) => (
                      <li key={svc.id}>
                        ...
                        >
                          <span className="font-mono text-paper">{svc.name}</span>
